...
public class ZKDatabase {
    ...
    /**
     * ZooKeeper会保存最近一段时间内执行的事务请求议案，个数限制默认为500个议案。上述committedLog就是用来保存议案的列表，
     *  上述maxCommittedLog表示最大议案的zxid，minCommittedLog表示committedLog中最小议案的zxid
     */
    ...
    public long loadDataBase() throws IOException {
        PlayBackListener listener=new PlayBackListener(){
            public void onTxnLoaded(TxnHeader hdr,Record txn){
                ...
                r.hdr = hdr;
                r.zxid = hdr.getZxid();
                addCommittedProposal(r);//添加到committedLog队列中
            }
        };
     ...
     * addCommittedProposal函数将请求添加至ZKDatabase的committedLog结构中，方便follower快速同步
     *
     * loadDataBase方法用于恢复数据，即从磁盘读取数据到内存，调用了addCommittedProposal方法，该方法维护了一个提交日志的队列，
     * 用于快速同步follower角色的节点信息，日志信息默认保存500条，所以选用了LinkedList队列用于快速删除数据溢出时的第一条信息
     */

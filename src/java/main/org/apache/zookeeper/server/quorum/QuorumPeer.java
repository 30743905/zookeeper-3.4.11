...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public static class QuorumServer {
        ...
        public int port=2888;//选举完之后，leader和learner的通信端口

        public int electionPort=-1;//选举之前，各server参与选举的port

        public long id;
        
        public LearnerType type = LearnerType.PARTICIPANT;//server类型，为PARTICIPANT 或者 OBSERVER;
    }
    /**
     ...
     */
    /**
     * LearnerType标记服务器是否能够参与投票，选举
     1、PARTICIPANT,能够参与一致性的投票，以及选举leader
     2、OBSERVER，只能观察
     */
     ...
     *
     *3.4.0之前选举leader时用udp协议的版本，现在已经Deprecated了
     */
    @Deprecated
    ...
    synchronized public void startLeaderElection() {
        ...
        if (electionType == 0) {//electionType默认配置为3，该if不会执行
            try {
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                listener.start();//Listener为一个线程，主要完成选举监听端口开启及处理连接到监听端口的Socket
        ...
        try {
            ...
            while (running) {
                ...
                case LOOKING:
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {//默认该if不会执行
     ...
     * CurrentEpoch是当前周期，AcceptedEpoch是即将成为的周期，
     前者相当于最终决定，后者相当于决定前的提议

     1、Leader类中先通过getEpochToPropose收集到一半以上的lastAcceptedEpoch，然后最大值+1成为新Leader的epoch，并设置给acceptedEpoch
     2、然后Leader将这个新epoch广播出去，收到一半以上ack确认消息后，将currentEpoch设置成这个新epoch
     3、Learner类收到2中广播的epoch时，修改自己的acceptedEpoch为新epoch，等Learner和Leader同步完成，会收到Leader的Leader.NEWLEADER报文，这时设置currentEpoch为新epoch
     */
